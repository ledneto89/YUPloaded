...
class _QuickDetailsPageState extends State<QuickDetailsPage> {
  ...
  Future<void> _yupload() async {
    ...
    try {
      ...
      if (brokerEmail.isNotEmpty) await EmailService.sendInvoice(brokerEmail: brokerEmail, loadNumber: loadNumber, pickupState: _pickupCity.isNotEmpty ? _pickupCity + ' ' + (_pickupState ?? '') : (_pickupState ?? ''), deliveryState: _deliveryCity.isNotEmpty ? _deliveryCity + ' ' + (_deliveryState ?? '') : (_deliveryState ?? ''), rate: 'USD ' + rate.toStringAsFixed(2), driverName: driverName, mcNumber: mcNumber, rateConUrl: widget.rateConUrl, bolUrl: widget.bolUrl, freightUrls: widget.freightUrls, podUrl: widget.podUrl, pickupZip: _pickupZipController.text.trim(), deliveryZip: _deliveryZipController.text.trim(), notes: _notesController.text.trim());
