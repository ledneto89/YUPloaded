...
if (_pickupState == null) { _showError('Please select pickup state'); return false; }
if (_deliveryState == null) { _showError('Please select delivery state'); return false; }
...
content: Text('You have used your 3 free loads. Upgrade to Pro for USD 19 per month.', style: GoogleFonts.barlow(fontSize: 14, color: textMuted)),
actions: [
...
TextButton(onPressed: () { Navigator.pop(ctx); Navigator.push(context, MaterialPageRoute(builder: (_) => const PaymentPage())); }, child: Text('UPGRADE', style: GoogleFonts.barlowCondensed(fontSize: 16, fontWeight: FontWeight.w900, color: orange))),
],
));
return false;
}
...
'userId': user.uid, 'loadNumber': loadNumber,
'mileage': int.tryParse(_mileageController.text) ?? 0,
'expenses': double.tryParse(_expensesController.text.replaceAll(',', '')) ?? 0.0,
'pickupState': _pickupState ?? '', 'deliveryState': _deliveryState ?? '',
'notes': _notesController.text.trim(), 'brokerEmail': brokerEmail,
'dispatcherEmail': _savedDispatcherEmail, 'rate': rate,
'status': 'invoiced', 'createdAt': FieldValue.serverTimestamp(),
});
...
if (brokerEmail.isNotEmpty) await EmailService.sendInvoice(brokerEmail: brokerEmail, loadNumber: loadNumber, pickupState: _pickupState ?? '', deliveryState: _deliveryState ?? '', rate: 'USD ' + rate.toStringAsFixed(2), driverName: driverName, mcNumber: mcNumber);
if (_savedDispatcherEmail.isNotEmpty) await EmailService.sendDispatcherPacket(dispatcherEmail: _savedDispatcherEmail, loadNumber: loadNumber, pickupState: _pickupState ?? '', deliveryState: _deliveryState ?? '', driverName: driverName);

if (mounted) {
await InvoiceGenerator.generateAndShare(loadNumber: loadNumber, driverName: driverName, mcNumber: mcNumber, pickupState: _pickupState ?? '', deliveryState: _deliveryState ?? '', rate: rate, brokerEmail: brokerEmail, context: context);
...
const SizedBox(height: 10),
Row(children: [Expanded(child: _stateDD('PICK UP STATE *', _pickupState, (v) => setState(() => _pickupState = v))), const SizedBox(width: 10), Expanded(child: _stateDD('DELIVERY STATE *', _deliveryState, (v) => setState(() => _deliveryState = v)))]),
const SizedBox(height: 10),
