...
bool get _allDone => _rateStatus != null && _bolStatus != null && _freightStatus != null && _podUploaded;
int get _completedCount => [_rateStatus != null, _bolStatus != null, _freightStatus != null, _podUploaded].where((s) => s).length;
...
if (url != null) {
setState(() { _podUrl = url; _podUploaded = true; });
} else {
if (mounted) {
ScaffoldMessenger.of(context).showSnackBar(
const SnackBar(content: Text('POD is required. Please upload your proof of delivery.'), backgroundColor: Color(0xFFEF4444)),
);
}
}
}

Widget _buildProgressSegment(bool done) {
...
child: Row(children: [
_buildProgressSegment(_rateStatus != null), const SizedBox(width: 6),
_buildProgressSegment(_bolStatus != null), const SizedBox(width: 6),
_buildProgressSegment(_freightStatus != null), const SizedBox(width: 6),
_buildProgressSegment(_podUploaded), const SizedBox(width: 6),
_buildProgressSegment(_allDone),
]),
),
...
child: Text('Take photos first then upload here. POD is required.', style: GoogleFonts.barlow(fontSize: 11, color: textMuted), textAlign: TextAlign.center),
),
Expanded(
...
: _yupNope(
() async { final url = await _uploadSinglePhoto('ratcon'); if (url != null) setState(() { _rateConUrl = url; _rateStatus = true; }); else setState(() => _rateStatus = true); },
...
const SizedBox(height: 10),
_buildDocSlot(icon: Icons.article, title: 'BILL OF LADING', status: _bolStatus,
onYup: () async { final url = await _uploadSinglePhoto('bol'); setState(() { if (url != null) _bolUrl = url; _bolStatus = true; }); },
onNope: () => setState(() => _bolStatus = false)),

const SizedBox(height: 10),

// FREIGHT PHOTOS - optional
Container(
padding: const EdgeInsets.all(14),
...
_buildSlotStatus(_freightStatus, _freightUrls.length.toString() + ' photo(s) uploaded', 'Skipped - your liability'),
if (_freightStatus == null) Text('Recommended for your protection', style: GoogleFonts.barlow(fontSize: 11, color: textMuted)),
])),
...
// POD - MANDATORY - no NOPE option
Container(
padding: const EdgeInsets.all(14),
decoration: BoxDecoration(
color: surface,
borderRadius: BorderRadius.circular(14),
border: Border.all(color: _podUploaded ? success : danger, width: 1.5),
),
child: Column(children: [
Row(children: [
Icon(Icons.check_circle, color: _podUploaded ? success : danger, size: 20),
const SizedBox(width: 12),
Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
Row(children: [
Text('PROOF OF DELIVERY', style: GoogleFonts.barlowCondensed(fontSize: 14, fontWeight: FontWeight.w800, color: textPrimary)),
const SizedBox(width: 8),
...
Text(_podUploaded ? 'Uploaded' : 'Must upload POD to continue', style: GoogleFonts.barlow(fontSize: 11, color: _podUploaded ? success : danger)),
])),
if (_podUploaded) Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: success.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(8)), child: Text('YUP', style: GoogleFonts.barlowCondensed(fontSize: 11, fontWeight: FontWeight.w800, color: success))),
]),
if (!_podUploaded) ...[
const SizedBox(height: 10),
GestureDetector(
...
child: Center(child: Text('YUP - UPLOAD POD', style: GoogleFonts.barlowCondensed(fontSize: 16, fontWeight: FontWeight.w900, color: background, letterSpacing: 1))),
),
),
],
if (_podUploaded) ...[
...
GestureDetector(
onTap: _uploadPOD,
child: Container(padding: const EdgeInsets.symmetric(vertical: 10), decoration: BoxDecoration(border: Border.all(color: success, width: 1.5), borderRadius: BorderRadius.circular(10)), child: Center(child: Text('REPLACE POD', style: GoogleFonts.barlowCondensed(fontSize: 13, fontWeight: FontWeight.w800, color: success, letterSpacing: 1)))),
),
],
]),
),
...
onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => QuickDetailsPage(
